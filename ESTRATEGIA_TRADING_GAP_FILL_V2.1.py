...
- UMBRALES MAS ESTRICTOS (v2.1.1):
   * Gap + estructura: 40% -> 50% (+10%)
   * Gap sin estructura: 50% -> 60% (+10%)
   * Sin gap: 60% -> 70% (+10%)
   * Resultado esperado: Win Rate 60-70%, menos senales falsas

================================================================================
SISTEMA DE PUNTUACION (v2.1)
================================================================================
...
AJUSTES POR ESTRUCTURA DE MERCADO:
...
UMBRALES DE OPERACION (ACTUALIZADOS - Mas estrictos):
| Escenario                              | Confianza Min. | Puntos Min.  |
|---------------------------------------|----------------|--------------|
| Gap + estructura correcta (vs PP)      | 50%           | 2.5/5.0      |
| Gap sin estructura optima              | 60%           | 3.0/5.0      |
| Sin gap, resistencia/soporte fuerte    | 70%           | 3.5/5.0      |

NOTA: Umbrales aumentados en v2.1.1 para reducir senales falsas
- Gap + estructura: 40% -> 50% (+10%)
- Gap sin estructura: 50% -> 60% (+10%)
- Sin gap: 60% -> 70% (+10%)
...
Cuando ejecutar:
  - 6:00-6:30 AM Tijuana (9:00-9:30 AM NY)
  - Antes de abrir cualquier operacion del dia
  - UNA SOLA VEZ al dia (no re-ejecutar durante el dia)

Workflow:
  1. Ejecutar script a las 6:15 AM
  2. REVISAR ESTRUCTURA DE MERCADO PRIMERO
  3. Revisar senales con confianza >=50%
  4. Verificar indicadores HTF en TradingView
  5. Colocar ordenes segun estructura
  6. Registrar en diario de trading

MODO 2: BACKTESTING
Uso: python script.py --backtest [dias] [confianza_min%] [simbolo]

Ejemplo: python script.py --backtest 30 50 BTC-USD
...
1. Position Sizing por Estructura:
   ...
   - Acumulacion/Distribucion fuerte: 2% capital
   - Consolidacion con acum/distrib: 1% capital
   - Consolidacion pura: 0% capital (NO OPERAR)
...
4. Limites Diarios:
   ...
   - Si pierdes 2 seguidas: STOP
   - Si ganas 3% capital: Considerar parar

5. Validacion Previa:
   - PAPER TRADING minimo 30 dias
   - Win Rate >55% antes de capital real
   - Profit Factor >1.5 consistente

6. IMPORTANTE - Horario de Ejecucion:
   - Ejecutar SOLO entre 6:00-6:30 AM Tijuana
   - NO re-ejecutar durante el dia (señales cambian)
   - Basar decisiones en analisis de 6 AM
   - Si necesitas actualizar, esperar al dia siguiente
...
def analyze_pre_ny(symbol):
    ...
    if gap_signal == "SHORT_TO_FILL":
        ...
        min_confidence = 2.5 if price_above_pp else 3.0
        ...
        min_confidence = 2.5 if price_below_pp else 3.0
...
if __name__ == "__main__":
    ...
    print("--- INICIO DE ANALISIS PRE-NY (Version 2.1.1 - Umbrales Actualizados) ---\n")
    print("NOTA: Umbrales mas estrictos - Menos senales, mayor calidad\n")
