...
class BinanceLiquidationAnalyzer:
    """Analiza liquidaciones usando fuentes alternativas compatibles con GitHub Actions"""
    
    # Usar API alternativa que no bloquea GitHub IPs
    ...
    def get_open_interest_alt(self, symbol):
        """Obtiene Open Interest de fuente alternativa (CoinGlass)"""
        try:
            # Convertir BTCUSDT -> BTC
            base_symbol = symbol.replace('USDT', '')
            
            url = f"{self.COINGLASS_API}/futures/openInterest/chart"
            params = {
                "symbol": base_symbol,
                "interval": "0"
            }
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and data.get('data'):
                    ...
                    return float(latest.get('openInterest', 0))
            
            return 0.0
        except Exception as e:
            print(f"  ⚠️ Error en Open Interest (Alt): {str(e)[:80]}")
            return 0.0
    
    def get_long_short_ratio_alt(self, symbol):
        """Obtiene ratio Long/Short de CoinGlass"""
        try:
            base_symbol = symbol.replace('USDT', '')
            
            url = f"{self.COINGLASS_API}/futures/longShortRate"
            params = {
                "symbol": base_symbol,
                "interval": "5m"
            }
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and data.get('data'):
                    ...
                    if isinstance(rates, list) and len(rates) > 0:
                        df_data = []
                        for item in rates[-30:]:  # Últimos 30 registros
                            long_rate = float(item.get('longRate', 0.5))
                            short_rate = float(item.get('shortRate', 0.5))
                            ratio = long_rate / short_rate if short_rate > 0 else 1.0
                            
                            df_data.append({
                                'longShortRatio': ratio,
                                'longAccount': long_rate,
                                'shortAccount': short_rate,
                                'timestamp': pd.to_datetime(item.get('createTime', 0), unit='ms')
                            })
            ...
            print(f"  ⚠️ Error en Long/Short Ratio (Alt): {str(e)[:80]}")
            return None
    
    def get_funding_rate_alt(self, symbol):
        """Obtiene funding rate de CoinGlass"""
        try:
            base_symbol = symbol.replace('USDT', '')
            
            url = f"{self.COINGLASS_API}/futures/fundingRate"
            params = {
                "symbol": base_symbol,
                "interval": "0"
            }
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and data.get('data'):
                    rates = data['data']
                    if isinstance(rates, list) and len(rates) > 0:
                        # Promediar funding de los principales exchanges
                        funding_values = []
                        for exchange_data in rates:
                            rate = float(exchange_data.get('rate', 0))
                            funding_values.append(rate)
                        
                        if funding_values:
                            return sum(funding_values) / len(funding_values)
            
            return 0.0
        except Exception as e:
            print(f"  ⚠️ Error en Funding Rate (Alt): {str(e)[:80]}")
            return 0.0
    
    def get_taker_volume_estimate(self, symbol):
        """Estima Buy/Sell ratio basado en datos agregados"""
        try:
            base_symbol = symbol.replace('USDT', '')
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and data.get('data'):
                    ...
                    if isinstance(liq_data, list) and len(liq_data) > 0:
                        df_data = []
                        for item in liq_data[-30:]:
                            buy_vol = float(item.get('buyVolUsd', 0))
                            sell_vol = float(item.get('sellVolUsd', 0))
            ...
            print(f"  ⚠️ Error en Buy/Sell estimation: {str(e)[:80]}")
            return None
    
    def get_open_interest(self, symbol):
        """Wrapper que intenta Binance primero, luego alternativa"""
        try:
        ...
        return self.get_open_interest_alt(symbol)
    ...
    def get_long_short_ratio(self, symbol, period='5m'):
        ...
        # Fallback a fuente alternativa
        return self.get_long_short_ratio_alt(symbol)
    
    def get_taker_volume(self, symbol, period='5m'):
        """Wrapper que intenta Binance primero, luego alternativa"""
        try:
            url = f"{self.BINANCE_API}/futures/data/takerlongshortRatio"
            params = {"symbol": symbol, "period": period, "limit": 30}
            response = self.session.get(url, params=params, timeout=5)
        ...
        # Fallback a estimación alternativa
        return self.get_taker_volume_estimate(symbol)
    
    def get_funding_rate(self, symbol):
        """Wrapper que intenta Binance primero, luego alternativa"""
        try:
            url = f"{self.BINANCE_API}/fapi/v1/fundingRate"
        ...
        # Fallback a fuente alternativa
        return self.get_funding_rate_alt(symbol)
